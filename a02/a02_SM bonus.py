...
#     off=rg.Curve.OffsetOnSurface(joined_outlines[i],surface,remapped[i],.001)
#     offset_outlines.append(off)
#     surfaces.append(surface)

# offset_outlines=th.list_to_tree(offset_outlines)
##########################################
...
for i in range(len(joined_outlines)):
    t_p=rg.Curve.ClosestPoint(moved_circles[i],rg.Curve.PointAt(joined_outlines[i],0),5)[1]
    rg.Curve.ChangeClosedCurveSeam(moved_circles[i],t_p)
    list_curves2=[joined_outlines[i], moved_circles[i]]
